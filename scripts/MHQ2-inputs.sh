...
# ARG_HELP([Extract MHQ2 fields from an unpacked UKB release (*.enc.ukb) to CSV format])
# ARGBASH_GO()
...
if [ ! -f $ukb_csv ]; then
    ...
    dconvert $_arg_enc_ukb csv -i$_arg_fields -e$_arg_encoding
    echo "Fields written to ${ukb_csv}"
else
    echo "${ukb_csv} aleady exists"
fi
