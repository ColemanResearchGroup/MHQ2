---
title: "MHQ2 Anorexia nervosa DSM-5 algoritm"
author: "Zhaoying Yu, Helena Davies, Christopher Huebel, Johan Z"
output: html_document
date: "2023-10-07"
---

```{r}
library("tidyverse")
```

# Import data

```{r}
dat <- readRDS("/Users/lilian/Desktop/MHQ2_Height_Alcohol_Field_Anonymous.rds")
#dat <- readRDS("/Users/christopherhuebel/work/raw_data/MHQ2_Height_Alcohol_Field_Anonymous.rds")
```

# Recode multiple choice questions
29130 Methods of controlling body shape or weight when at this low weight
1	Made yourself vomit
2	Used laxatives (pills or liquids)
3	Used diuretics (water pills)
4	Used weight loss pills
5	Exercised excessively or felt distressed if unable to exercise
6	Fasted or not eaten for eight waking hours or more
7	Other methods to lose weight/stay at low weight
0	None of above
-3	Prefer not to answer

```{r made yourself vomit}
dat <- dat %>%
  mutate(
    made_yourself_vomit =
      case_when(
        `29130-0.0` == 1 |
        `29130-0.1` == 1 |
        `29130-0.2` == 1 | 
        `29130-0.3` == 1 |
        `29130-0.4` == 1 |
        `29130-0.5` == 1 |
        `29130-0.6` == 1 ~ 1,
                     
        # did not have any weight control behaviour
        `29130-0.0` == 0 ~ 0,
        
        # people who have not answered the MHQ2
        TRUE ~ NA_real_
                     )
         )

# Check (expecting 1.021)
dat %>%
  count(made_yourself_vomit)
```

```{r used laxatives}
dat <- dat %>%
  mutate(
    used_laxatives =
      case_when(
        `29130-0.0` == 2 |
        `29130-0.1` == 2 |
        `29130-0.2` == 2 | 
        `29130-0.3` == 2 |
        `29130-0.4` == 2 |
        `29130-0.5` == 2 |
        `29130-0.6` == 2 ~ 1,
                     
        # did not have any weight control behaviour
        `29130-0.0` == 0 ~ 0,
        
        # people who have not answered the MHQ2
        TRUE ~ NA_real_
                     )
         )

# Check (expecting 1.050)
dat %>%
  count(used_laxatives)
```

```{r used diuretics}
dat <- dat %>%
  mutate(
    used_diuretics =
      case_when(
        `29130-0.0` == 3 |
        `29130-0.1` == 3 |
        `29130-0.2` == 3 | 
        `29130-0.3` == 3 |
        `29130-0.4` == 3 |
        `29130-0.5` == 3 |
        `29130-0.6` == 3 ~ 1,
                     
        # did not have any weight control behaviour
        `29130-0.0` == 0 ~ 0,
        
        # people who have not answered the MHQ2
        TRUE ~ NA_real_
                     )
         )

# Check (expecting 247)
dat %>%
  count(used_diuretics)
```

```{r used weight loss pills}
dat <- dat %>%
  mutate(
    used_weight_loss_pills =
      case_when(
        `29130-0.0` == 4 |
        `29130-0.1` == 4 |
        `29130-0.2` == 4 | 
        `29130-0.3` == 4 |
        `29130-0.4` == 4 |
        `29130-0.5` == 4 |
        `29130-0.6` == 4 ~ 1,
                     
        # did not have any weight control behaviour
        `29130-0.0` == 0 ~ 0,
        
        # people who have not answered the MHQ2
        TRUE ~ NA_real_
                     )
         )

# Check (expecting 288)
dat %>%
  count(used_weight_loss_pills)
```

```{r excessive exercise}
dat <- dat %>%
  mutate(
    excessive_exercise =
      case_when(
        `29130-0.0` == 5 |
        `29130-0.1` == 5 |
        `29130-0.2` == 5 | 
        `29130-0.3` == 5 |
        `29130-0.4` == 5 |
        `29130-0.5` == 5 |
        `29130-0.6` == 5 ~ 1,
                     
        # did not have any weight control behaviour
        `29130-0.0` == 0 ~ 0,
        
        # people who have not answered the MHQ2
        TRUE ~ NA_real_
                     )
         )

# Check (expecting 1,917)
dat %>%
  count(excessive_exercise)
```

```{r fasten}
dat <- dat %>%
  mutate(
    fasten =
      case_when(
        `29130-0.0` == 6 |
        `29130-0.1` == 6 |
        `29130-0.2` == 6 | 
        `29130-0.3` == 6 |
        `29130-0.4` == 6 |
        `29130-0.5` == 6 |
        `29130-0.6` == 6 ~ 1,
                     
        # did not have any weight control behaviour
        `29130-0.0` == 0 ~ 0,
        
        # people who have not answered the MHQ2
        TRUE ~ NA_real_
                     )
         )

# Check (expecting 2,403)
dat %>%
  count(fasten)
```

```{r other weight control}
dat <- dat %>%
  mutate(
    other_weight_control =
      case_when(
        `29130-0.0` == 7 |
        `29130-0.1` == 7 |
        `29130-0.2` == 7 | 
        `29130-0.3` == 7 |
        `29130-0.4` == 7 |
        `29130-0.5` == 7 |
        `29130-0.6` == 7 ~ 1,
                     
        # did not have any weight control behaviour
        `29130-0.0` == 0 ~ 0,
        
        # people who have not answered the MHQ2
        TRUE ~ NA_real_
                     )
         )

# Check (expecting 823)
dat %>%
  count(other_weight_control)
```

# BMI at low weight

Lowest weight during period when underweight [metric] / Height [cm]/100)^2

```{r}
dat <- dat %>%
  mutate(BMI =
           case_when(is.na(`29125-0.0`) |
                     is.na(`50-0.0`) |
                     `29125-0.0` %in% c(0, -1, -3) ~ NA,
                     TRUE ~ `29125-0.0` / (`50-0.0`/100)^2)
        )
```
              
# Anorexia nervosa

```{r}
#Anorexia nervosa
dat <- dat %>%
  mutate(Anorexia_nervosa = 

           ##BMI at low weight <= 18.55 kg/m2
                       BMI <= 18.55 &

           #Did you feel fat? = 1 Yes
                       (`29122-0.0` == 1 |

           #During the time when you were at this low weight: Did you think or feel that your body or parts of your body were larger than they actually were? = 1 Yes
                        `29124-0.0` == 1 |

           #Did/do you ever think your low weight had/has negative consequences for your health? = 0 No
                        `29128-0.0` == 0 |

           #Was your self-esteem dependent on your body shape or weight = 2 A great deal OR 1 A moderate amount
                        `29129-0.0` %in% c(1,2)) &

           #Afraid that you might gain weight or become fat = 1 Yes
                       (`29124-0.0` == 1 |

           #Have you done any of the following as a way to control your body shape or weight? = any of 1 to 7
                        made_yourself_vomit == 1 |
                        used_laxatives == 1 |
                        used_diuretics == 1 |
                        used_weight_loss_pills == 1 |
                        excessive_exercise == 1 |
                        fasten == 1 |
                        other_weight_control == 1)
            ~ 1,
         TRUE ~ 0
  )
)
```

# Anorexia nervosa - binge/purge subtype

```{r}
#Anorexia nervosa - binge/purge subtype
dat <- dat %>%
  mutate(Anorexia_nervosa_binge_eating_purging =
#Criteria A
           #BMI at low weight <= 18.55 kg/m2
              case_when( BMI <= 18.55 &
#Criteria B
           #Afraid that you might gain weight or become fat = 1 Yes
                       (`29124-0.0` == 1 |

          #Have you done any of the following as a way to control your body shape or weight? = any of 1 to 6
                        made_yourself_vomit == 1 |
                        used_laxatives == 1 |
                        used_diuretics == 1 |
                        used_weight_loss_pills == 1 |
                        excessive_exercise == 1 |
                        fasten == 1 |
                        other_weight_control == 1) &

#Criteria C
           #Did you feel fat? = 1 Yes
                       (`29122-0.0` == 1 |

           #During the time when you were at this low weight: Did you think or feel that your body or parts of your body were larger than they actually were? = 1 Yes
                        `29124-0.0` == 1 |

           #Did/do you ever think your low weight had/has negative consequences for your health? = 0 No
                        `29128-0.0` == 0 |

           #Was your self-esteem dependent on your body shape or weight = 2 A great deal OR 1 A moderate amount
                        `29129-0.0` %in% c(1,2)) &

#ANBP criteria
          #Have you had recurrent episodes of excessive overeating or binge eating (EP6c) = 2 Yes, ONLY at time(s) of low weight OR 1 Yes, BOTH at time(s) of low weight AND at time(s) when I was not at low weight)
                       (`29134-0.0` %in% c(1,2) |

          #Have you done any of the following as a way to control your body shape or weight (EP5) = 1 Made yourself vomit OR 2 Used laxatives (pills or liquids meant to stimulate bowel movement) OR 3 Used diuretics (water pills) 
                        made_yourself_vomit == 1 |
                        used_laxatives == 1 |
                        used_diuretics == 1)
            ~ 1,
         TRUE ~ 0
  )
)
```

# Anorexia nervosa - restricting subtype

```{r}
#Anorexia nervosa - restricting subtype
dat <- dat %>%
  mutate(Anorexia_nervosa_restricting =
#Criteria A
           #BMI at low weight <= 18.55 kg/m2
              case_when( BMI <= 18.55 &
#Criteria B
           #Afraid that you might gain weight or become fat = 1 Yes
                       (`29124-0.0` == 1 |

          #Have you done any of the following as a way to control your body shape or weight? = 5 exercise excessively OR 6 fasten
                        excessive_exercise == 1 |
                        fasten == 1) &

#Criteria C
           #Did you feel fat? = 1 Yes
                       (`29122-0.0` == 1 |

           #During the time when you were at this low weight: Did you think or feel that your body or parts of your body were larger than they actually were? = 1 Yes
                        `29124-0.0` == 1 |

           #Did/do you ever think your low weight had/has negative consequences for your health? = 0 No
                        `29128-0.0` == 0 |

           #Was your self-esteem dependent on your body shape or weight = 2 A great deal OR 1 A moderate amount
                        `29129-0.0` %in% c(1,2)) &

            ~ 1,
         TRUE ~ 0
  )
)
```
