...
```{r}
dat <- dat %>%
  mutate(Anorexia_nervosa = case_when(
    # Criterion A
...
```{r}
dat <- dat %>%
  mutate(Anorexia_nervosa_binge_eating_purging = case_when(
    # Criterion A
    # BMI at low weight <= 18.55 kg/m2
    BMI_at_low_weight <= 18.55 &
      # Criterion B
...
```{r}
dat <- dat %>%
  mutate(Anorexia_nervosa_restricting = case_when(
    # Criterion A
    # BMI at low weight <= 18.55 kg/m2
    BMI_at_low_weight <= 18.55 &
      # Criterion B
