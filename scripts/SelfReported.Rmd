...
packages <- c("data.table","tidyverse","dplyr", "readr")
...
MHQ2 <- fread(file = params$data, data.table = FALSE)
...
```{r Output}
SelfReported <- MHQ2 |> select(eid, starts_with("SelfReported"))

write_tsv(SelfReported, params$output)
```