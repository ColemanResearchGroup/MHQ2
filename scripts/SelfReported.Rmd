...
MHQ2 <- fread(args[1], data.table = FALSE)
...
grep_reports <- function(data, fields) {
  return(
    apply(X = data,
      MARGIN = 1,
      FUN = function(selected_row) {
        case_when(all(is.na(selected_row)) == TRUE ~ NA_real_,
          sum(fields %in% selected_row) > 0 ~ 1,
          sum(fields %in% selected_row) == 0 ~ 0
        )
      }
    )
  )
