...
define_age <- function(data, date.field, cohort){
  ...
  if(cohort == "UKB"){
    as.numeric(
	  ...
	  ceiling(
	    difftime(
	      median(as.Date(date.field), na.rm = TRUE),
	      as.Date(`53-0.0`),
	      units="days"
	    )
	  /365.25
...
define_age_bands <- function(data, age.variable){
  return(
    ordered(
      with(data,
        case_when(is.na(age.variable) ~ NA_real_,
	  age.variable >= 45 & age.variable <= 54 ~ "45to54",
	  age.variable >= 55 & age.variable <= 64 ~ "55to64",
	  age.variable >= 65 & age.variable <= 74 ~ "65to74",
	  age.variable >= 75 & age.variable <= 84 ~ "75to84",
	  age.variable >= 85 ~ "85Plus",
	  .default ~ NA_real_
	)
      )
...
define_sex <- function(data, cohort.identifier){
  return(
    with(data,
      ifelse(!is.na(cohort.identifier),
        `31-0.0`,
	NA
      )
    )
...
message("Measure MHQ2 MHQ1 MHQ2-MHQ1 UKB MHQ2-UKB")
