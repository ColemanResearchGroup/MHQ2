...
if(diagnosticsFlag) summary(anx_pad_data$Gad7Score)
...
```{r}
# Create a new column named 'GadCurrent' in the data frame
# Calculate 'GadCurrent' value.
anx_pad_data <- anx_pad_data %>%
  mutate(
    GadCurrent = case_when(
      # If 'Gad7Score' is missing (NA) Assign NA to the 'GadCurrent' column.
      is.na(Gad7Score) ~ NA_real_,
      # If 'Gad7Score' is greater than 9 (i.e. >=10 for moderate current Gad)
      # Assign 1 to the 'GadCurrent' column (as case)
      Gad7Score > 9 ~ 1,
      ...
      # Assign 0
      Gad7Score < 10 & Gad7Score + Gad7NaCount * 3 < 10 ~ 0,
...
```{r}
anx_pad_data <- anx_pad_data %>%
  mutate(PanicAttacksEver = case_when(
...
```{r}
anx_pad_data <- anx_pad_data %>%
  mutate(PanicDisorder = case_when(
    # Symptoms {panic attack ever}
    PanicAttacksEver == 1 &
      (
