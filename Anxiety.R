...
anx_pad_data <- dat %>% 
  select(ID,
         matches(paste(anx_pad_field_numbers, 
                       collapse = "|")))
...
setDT(dat,key = "ID")
setDT(anx_pad_data,key = "ID")

dat[anx_pad_data, on=c('ID'),c('Gad7Score','GadCurrent','PanicSymptomsCount','PanicAttacksEver','PanicDisorder') :=list(i.Gad7Score,i.GadCurrent,i.PanicSymptomsCount,i.PanicAttacksEver,i.PanicDisorder)]
